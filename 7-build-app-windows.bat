...
call reportgenerator "-title:Recipe Library Documentation Coverage Report (Windows)" "-reports:**/lcov_doxygen_lib_win.info" "-targetdir:docs/coverxygenlibwin" "-reporttypes:Html" "-filefilters:-*.md;-*.xml;-*[generated];-*build*" "-historydir:report_doc_lib_hist_win"
...
call reportgenerator "-title:Recipe Library Test Documentation Coverage Report (Windows)" "-reports:**/lcov_doxygen_test_win.info" "-targetdir:docs/coverxygentestwin" "-reporttypes:Html" "-filefilters:-*.md;-*.xml;-*[generated];-*build*" "-historydir:report_doc_test_hist_win"
...
echo Generate Test Coverage Data for Recipe Tests
if exist "build_win\build\Debug\recipe_tests.exe" (
    call OpenCppCoverage.exe --export_type=cobertura:recipe_tests_unit_win_cobertura.xml --sources include --sources src --sources tests -- build_win\build\Debug\recipe_tests.exe
    
    echo Filter coverage XML to include only project files
    PowerShell -ExecutionPolicy Bypass -NoProfile -Command "if (Test-Path 'recipe_tests_unit_win_cobertura.xml') { $xml = [xml](Get-Content 'recipe_tests_unit_win_cobertura.xml'); $package = $xml.coverage.packages.package; $classes = $package.classes; $projectFiles = @('graph.c', 'hash_table.c', 'heap.c', 'linked_list.c', 'menu.c', 'queue.c', 'recipe.c', 'sparse_matrix.c', 'stack.c', 'xor_linked_list.c'); $totalCovered = 0; $totalValid = 0; $classesToRemove = @(); foreach ($class in $classes.class) { if ($class.name -notin $projectFiles) { $classesToRemove += $class } else { $lines = $class.lines; if ($lines) { foreach ($line in $lines.line) { $hits = [int]$line.hits; if ($hits -gt 0) { $totalCovered++ }; $totalValid++ } } } }; foreach ($class in $classesToRemove) { [void]$classes.RemoveChild($class) }; $lineRate = if ($totalValid -gt 0) { $totalCovered / $totalValid } else { 0.0 }; $xml.coverage.SetAttribute('line-rate', [string]$lineRate); $xml.coverage.SetAttribute('lines-covered', [string]$totalCovered); $xml.coverage.SetAttribute('lines-valid', [string]$totalValid); $package.SetAttribute('line-rate', [string]$lineRate); $xml.Save((Resolve-Path 'recipe_tests_unit_win_cobertura.xml')); Write-Host 'Filtered coverage XML:' ([math]::Round($lineRate * 100, 2)) '% (' $totalCovered '/' $totalValid ')' }"
    
    echo Generate Unit Test Coverage Report
    call reportgenerator "-title:Recipe Library Unit Test Coverage Report (Windows)" "-targetdir:docs/coveragereportlibwin" "-reporttypes:Html" "-reports:**/recipe_tests_unit_win_cobertura.xml" "-sourcedirs:include;src;tests" "-filefilters:-*minkernel\*;-*gtest*;-*a\_work\*;-*gtest-*;-*gtest.cc;-*gtest.h;-*build*" "-historydir:report_test_hist_win"
    call reportgenerator "-targetdir:assets/codecoveragelibwin" "-reporttypes:Badges" "-reports:**/recipe_tests_unit_win_cobertura.xml" "-sourcedirs:include;src;tests" "-filefilters:-*minkernel\*;-*gtest*;-*a\_work\*;-*gtest-*;-*gtest.cc;-*gtest.h;-*build*"
) else (
    echo Warning: recipe_tests.exe not found, skipping code coverage generation
)
...
echo Package Publish Windows Binaries
if exist "include" (
    call robocopy include "build_win\build\Release\include" /E
)
...
if exist "include" (
    call robocopy include "build_win\build\Debug\include" /E
)
